...
def load_schedule():
    return pd.read_csv(SCHEDULE_PATH, parse_dates=['Date'], infer_datetime_format=True)
...
def main(args):
    odds = None
    if args.odds:
        if args.odds.lower() == "polymarket":
            odds = PolymarketOddsProvider().get_odds()
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-odds', help='Sportsbook to fetch from. (fanduel, draftkings, betmgm, pointsbet, caesars, wynn, bet_rivers_ny, polymarket)')
